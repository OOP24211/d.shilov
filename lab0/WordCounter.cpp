...
WordCounter::WordCounter() : wordCount_(0) {} //оставил для примера себе, понял что у int zero-initialization

void WordCounter::calculateFreq() {
    for(auto& it : table_) {
        it.second.precent = (static_cast<float>(it.second.count) / wordCount_) * 100;
    }
}
